...
// ── ADMIN PANEL ──────────────────────────────────────────
function AdminPanel({ menu, shopId, saving, shopInfo, setShopInfo }) {
...
useEffect(() => {
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", bottom: 16, left: "50%", transform: "translateX(-50%)", background: "#1a1008", borderRadius: 999, padding: "6px 8px", display: "flex", gap: 4, zIndex: 300, boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
        {[["customer", "👁 菜单"], ["admin", "⚙️ 后台"]].map(([v, label]) => (
          <button key={v} onClick={() => setView(v)} style={{
            ...
            fontFamily: "'Noto Serif SC', serif", fontSize: "0.8rem", transition: "all 0.2s"
          }}>{label}</button>
        ))}
      </div>

      {view === "customer"
        ? <MenuView menu={menu} lang={lang} setLang={setLang} shopInfo={shopInfo} />
        : <AdminPanel menu={menu} shopId={shopId} saving={saving} shopInfo={shopInfo} setShopInfo={setShopInfo} />
      }
    </>
  );
}
